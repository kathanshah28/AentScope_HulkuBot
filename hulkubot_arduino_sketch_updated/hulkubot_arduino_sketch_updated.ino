      ...
      if (Serial.read() == 0xAA) {
        ...
        // ---------------------------------
          ...
          for (int id = 1; id <= 6; id++) {
            ...
            // example: - for motor 1: - pulse is 1500 hex will be 0x05DC -> binary 0000010111011100
            if (Wire.available() >= 2) {
              uint8_t msb = Wire.read();    // reads msb -> 00000101  ->  hex 0x05
              uint8_t lsb = Wire.read();    // reads lsb -> 11011100  ->  hex 0xDC
              // msb -> 00000101 appending prefix 00000000 for making it 16 bit // msb << 8 -> 0000010100000000(removing 8 bits from left and appening zero's behind)
              // lsb -> 11011100 appending prefix 00000000 for makinh it 16 bit // lsb -> 0000000011011100
              // actual operation now pos -> 0000010100000000 or 0000000011011100 -> 0000010111011100 => 0x05DC
              pos = (msb << 8) | lsb;        
            }
            
            // Pack MSB and LSB into our 15-byte response
            int idx = 3 + (id - 1) * 2;  // It will give every odd number till 15 for msb byte result of each motor like 3, 5, 7, 9, 11, 13
            // msb byte transfer
            // pos -> 0000010111011100 -> pos >> 8 -> 0000000000000101(removing 8 bits from right and appening the same number as in the first left here 0)
            // 0xFF appending prefix 00000000 for 16 bit process => 0000000011111111  
            resp[idx] = (pos >> 8) & 0xFF;  // 0000000000000101 & 0000000011111111 => 0000000000000101
            // lsb byte transfer
            // pos -> 0000010111011100 
            resp[idx+1] = pos & 0xFF;  // 0000010111011100 & 00000000011111111 => 0000000011011100
          }
